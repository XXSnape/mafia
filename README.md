# 🎩Телеграм бот для игры в [Мафию](https://ru.wikipedia.org/wiki/Мафия_(игра)) с большим количеством игроков и ролей (<u>более 35!</u>). 🤖AI ассистент поможет новичкам быстро разобраться с правилами и настройками бота
...
AI ассистент - RAG система, обученная на полной документации, отвечающая на вопросы пользователя об игре, настройках, режимах.

Чтобы им воспользоваться, нужно ввести /q и задать любой вопрос

## ⚙️Установка
❗Так как установка AI пакетов может быть затруднительна из-за вычислительных ресурсов, большого количества употребляемой памяти, загрузка по умолчанию происходит без AI-ассистента.

1) 🐳Через docker без AI
...
обязательно дополнив своими данными
...
2) 👾Через докер с AI
* После установки docker и клонирования репозитория необходимо подкорректировать пару файлов:
- В Dockerfile замените 14 строку "RUN poetry install --without lint,ai" на "RUN poetry install --without lint" (это добавит пакеты ai)
- В .env впишите переменную `AI_USE` со значением "True", а также впишите API ключ для доступа к бесплатной модели deepseek-r1-0528:free [на этом ресурсе](https://openrouter.ai/settings/keys) в переменную `AI_DEEPSEEK_API_KEY`
* Запустите контейнеры
```sh
docker compose up
```

3) 🌐Частично через docker и локально с AI

😭Дело в том, что скачивание пакетов для AI в контейнерах может быть чересчур затратным по времени (около 15 минут) и памяти (28 гб на контейнер), поэтому в некоторых случаях имеет смысл запустить бот и фоновые задачи на своей машине

* Для работы с AI нужно поменять переменные в .env, как описано в пункте 2
* Для локальной работы с базами данных и очередью замените переменные `DB_HOST`, `RABBITMQ_HOST`, `REDIS_HOST` на значение "localhost"
* Дальше необходимо [создать виртуальное окружение](https://docs.python.org/3/library/venv.html) для python 3.12 локально и установить poetry и пакеты для работы
```sh
pip install poetry
```
...
⚠️Эта команда может занять некоторое время

* Запустите контейнеры для работы с базами данных и очередью
```sh
docker compose up redis postgres rabbitmq
```

* После успешного запуска всех сервисов можно переходить к запуску самого бота
```sh
cd bot
```
...
* Для корректной работы со ставками, уведомлениями и т.п. запустите в другом терминале фоновые задачи
