...
class BaseRouter(RouterHelper):
    ...
    async def how_to_set_up_game(self):
        text = (
            ...
            f"«{Groupings.criminals.value.name}» и роли из других группировок, которые имеют союзников, заместителей. Чтобы отключить такую возможность для ролей, не входящих в группировку «{Groupings.criminals.value.name}», нужно выключить этот параметр\n\n"
            "🛠️Другие настройки\n\n"
            ...
            f"● Маршал может убивать ночью — у данной роли есть 2 опции на выбор: проверить или стрелять. Чтобы оставить только первую опцию, отключите данную настройку. (Подробнее о возможностях Маршала в разделе «{ROLES_SELECTION}»)\n\n"
            ...
            "нная настройка, и все игроки успели сделать ход, ночь может завершиться через секунд 20. То же самое с этапом голосования\n\n"
