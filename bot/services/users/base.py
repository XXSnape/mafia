...
class BaseRouter(RouterHelper):
    ...
    async def how_to_start_game(self):
        text = (
            ...
            "🎟️Присоединиться к игре или выйти из нее можно "
            "также нажав на кнопку под этим сообщением.\n\n"
            "❗️Минимум в игре могут участвовать 4 человека, "
    ...
    async def what_are_bids(self):
        text = (
            ...
            f"такие: за {make_pretty(Instigator.role)} максимальная ставка 500, за {make_pretty(Warden.role)} 600, тогда 6ой ролью будет "
            f"{make_pretty(Warden.role)}.\n\n"
            f"🔥В случае досрочного завершения игры (сбой телеграм, расформирование группы во время игры и т.д.) "
            f"деньги за сделанные ставки вернутся в полном объеме."
        )
        await self.callback.message.edit_text(
            text=make_build(text),
            reply_markup=generate_inline_kb(
                data_with_buttons=[HOW_TO_SET_UP_GAME_BTN, HELP_BTN]
            ),
        )
    ...
    async def how_to_play(self):
        text = (
            ...
            "знакомит с союзниками при наличии и в некоторых случаях с сокомандниками по группировке.\n\n"
            "🌃2) Наступление ночи\n\n"
            ...
            "👿Проигнорировать это сообщение за игру можно 1 раз. В случае повторного невыполнения "
            ...
            "Этот этап пропускается, если нет кандидата, набравшего бОльшее количество голосов "
            "на предыдущем шаге.\n"
    ...
    async def how_to_set_up_game(self):
        text = (
            ...
            "🛠Виды настроек:\n\n"
            "❌1) Забаненные роли\n\n"
            '😁Нет обязательств играть сразу со всеми ролями. Если кто-то хочет поиграть в "самую обыкновенную" мафию '
            "с 4 ролями (мафия, мирный, доктор, комиссар) но с компанией из 20 человек, такая возможность есть!\n\n"
            "Бот пришлет интерфейс, с помощью которого можно выбрать, какие роли в игре не смогут участвовать. "
            ...
            "❗️«Базовые роли» переопределять нельзя, ведь они должны участвовать в каждой игре. Информация "
    ...
    async def how_to_see_statistics(self):
        text = (
            ...
            f"группы бот пришлет в общий чат информацию о группе и рейтингу игроков"
        )
        await self.callback.message.edit_text(
            text=make_build(text),
            reply_markup=to_help_kb(),
